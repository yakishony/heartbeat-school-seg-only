...
def plot_signal_on_ax(ax, signal, y, s=1, show_labels=True):
...
def plot_recording(rec_id, dataset, name, xl=None, yl=None, sa_fig: bool = False):
    ...
    plot_signal_on_ax(ax, data["signal"], data["y"])
    if xl:
        ax.set_xlim(xl)
    if yl:
        ax.set_ylim(yl)
...
def plot_recording_before_and_after(rec_id, dataset_before, dataset_after,
    ...
    t_before = np.arange(len(sig_before)) / RATE
    t_after = np.arange(len(sig_after)) / RATE
